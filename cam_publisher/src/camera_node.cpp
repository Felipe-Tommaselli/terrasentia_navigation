...
/*
@brief: Publish compressed image to ROS topic
@param: Camera image, Compressed image publisher, CameraInfo publisher, Topic name, Image header, CameraInfo message, frame count
*/
void image_publisher(cv::Mat &cam_image, ros::Publisher &cam_pub, ros::Publisher &cam_info_pub, const std::string &topic_name, std_msgs::Header &image_header, sensor_msgs::CameraInfoPtr &cam_info, int frame_count)
{
    if (!cam_image.empty())
    {
        ...
        // Encode image as JPEG compressed
        sensor_msgs::CompressedImage cam_msg;
        cam_msg.header = image_header;
        cam_msg.format = "jpeg";

        std::vector<uchar> buf;
        std::vector<int> params = {cv::IMWRITE_JPEG_QUALITY, 90};
        cv::imencode(".jpg", undistorted_image, buf, params);
        cam_msg.data = buf;
        ...
        if (frame_count % 10 == 0)
        {
            cam_info_pub.publish(new_cam_info);
        }
    }
    else
    {
...
int main(int argc, char **argv)
{
    ...
    bool mjpeg_set = cam.cap.set(cv::CAP_PROP_FOURCC, cv::VideoWriter::fourcc('M', 'J', 'P', 'G'));
    
    if (!mjpeg_set) {
        ROS_WARN("Could not force MJPEG format. USB Bandwidth might be saturated!");
    } else {
        ROS_INFO("Successfully set MJPEG format.");
    }
    ...
    ros::Publisher cam_pub = nh.advertise<sensor_msgs::CompressedImage>(topic_name + "/image_rect_color/compressed", 1);
    ...
    while (ros::ok() && cam.isOpened())
    {
        ...
        image_publisher(cam_image, cam_pub, cam_info_pub, topic_name, image_header, cam_info, frame_count);
